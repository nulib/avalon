...
resource "aws_ssm_parameter" "encoding-presets-path" {
  type    = "String"
  name    = "${local.settings_prefix}/encoding/presets_path"
  value   = "config/media_convert_presets/"
  }
...
resource "aws_ssm_parameter" "redis-url" {
  type    = "SecureString"
  name    = "${local.settings_prefix}/redis/url"
  value   = "redis://${module.data_services.outputs.redis.address}:${module.data_services.outputs.redis.port}/"
  }
...
resource "aws_ssm_parameter" "streaming-content_path" {
  type    = "String"
  name    = "${local.settings_prefix}/streaming/content_path"
  value   = "/"
  }
