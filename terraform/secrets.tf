...
module "secrets" {
  ...
  defaults  = jsonencode({
    ...
    avr_certificate_domain          = "*.${trimsuffix(module.core.outputs.vpc.public_dns_zone.name, ".")}"
    canvas_api_token                = ""
    domain_host                     = "avr.${module.core.outputs.vpc.public_dns_zone.name}"
    streaming_aliases               = []
    streaming_certificate_domain    = "*.${trimsuffix(module.core.outputs.vpc.public_dns_zone.name, ".")}"
